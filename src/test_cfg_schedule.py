"""Test CFG scheduling: use guidance only on select steps."""
...
def generate_cfg_scheduled(model, task, num_step, cfg_schedule, pos_temp=5.0, seed=42):
    """_generate_iterative with per-step CFG. Steps with cfg=0 skip uncond pass."""
...
def main():
    ...
    schedules = {
        "all_cfg3":     [3.0]*8,
        "all_cfg0":     [0.0]*8,
        "first1_cfg3":  [3.0, 0,0,0,0,0,0,0],
        "first2_cfg3":  [3.0, 3.0, 0,0,0,0,0,0],
        "first3_cfg3":  [3.0, 3.0, 3.0, 0,0,0,0,0],
        "first4_cfg3":  [3.0, 3.0, 3.0, 3.0, 0,0,0,0],
        "last2_cfg3":   [0,0,0,0,0,0, 3.0, 3.0],
        "last4_cfg3":   [0,0,0,0, 3.0, 3.0, 3.0, 3.0],
        "bookend":      [3.0, 0,0,0,0,0,0, 3.0],
        "taper":        [3.0, 2.0, 1.0, 0,0,0,0,0],
        "first2_cfg1":  [1.0, 1.0, 0,0,0,0,0,0],
    }
    ...
    for sname, sched in schedules.items():
        ...
        for ti, text in enumerate(texts):
            ...
            path = f"/tmp/cfgsched_{sname}_{ti}.wav"
            torchaudio.save(path, audio.unsqueeze(0), 24000)
            segs2, _ = whisper.transcribe(path, language="en")
            transcript = " ".join(s.text.strip() for s in segs2)
            w = wer(text, transcript)
            wers.append(w)
            fwd = fp
        n_cfg = sum(1 for s in sched if s > 0)
        print(f"{sname:>15s}: fwd={fwd:2d} cfg_steps={n_cfg} avg_WER={np.mean(wers)*100:3.0f}%")
